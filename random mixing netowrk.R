...
param <- param.net(vaccine.rate=0, #vaccination rate
 ...
 init <- init.net(i.num = 1, v.num =0*24 ,status.rand = FALSE)
...
# formation <- ~edges #expected number of edges given a mean degree of x
# target.stats <- c(7.3*24/2) #87.6
# coef.diss<-dissolution_coefs(dissolution= ~offset(edges), duration=1)

## ----netEst1, results = "hide"-------------------------------------------
est1 <- netest(nw, formation=~edges, target.stats=87.6,coef.diss=dissolution_coefs(~offset(edges), duration=1))
